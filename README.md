...
Contributing to Git seems hard to start. The codebase is huge, there's no
...
comment in the Git source tree, grouped by directory, with the author and date
of the last change to that line and a link straight to it on GitHub.
...
but the codebase is full of notes left behind. Part of the motivation is seeing
...
- **Dates are the last time the line was touched, not when the marker was
  added.** A `NEEDSWORK` from 2011 sitting on a line that was reindented in
  2024 will show 2024.
- **A marker is not a ticket.** Some are notes-to-self, some are already
  obsolete, and some are hard. Read the surrounding code and the commit that
...
- **Check the mailing list first.** Search
  ...
  working on it right now. You don't need to subscribe to search. It's public,
  and reading it is how you find out whether a thing is worth doing.
  Additionally, to contribute to Git, you will have to deal with the mailing
  list.

## Running it yourself

You can read [WhatToGit.md](WhatToGit.md) directly. Running it yourself is
intended more for the development of the tool.
...
A shallow clone will not work. `git blame` needs history; without it, every
line is attributed to the most recent commit and all the dates come out wrong.
